namespace AdventOfCode2021.Tests

open Xunit
open AdventOfCode2021.Day18

module Day18Tests =
    
    let testInput = 
        [|
        "[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]"
        "[[[5,[2,8]],4],[5,[[9,9],0]]]"
        "[6,[[[6,2],[5,6]],[[7,6],[4,7]]]]"
        "[[[6,[0,7]],[0,9]],[4,[9,[9,0]]]]"
        "[[[7,[6,4]],[3,[1,3]]],[[[5,5],1],9]]"
        "[[6,[[7,3],[3,2]]],[[[3,8],[5,7]],4]]"
        "[[[[5,4],[7,7]],8],[[8,3],8]]"
        "[[9,3],[[9,9],[6,[4,9]]]]"
        "[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]"
        "[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]"
        |]

    [<Fact>]
    let ``Day 18 Puzzle 1`` () =
        Assert.Equal(4140, puzzle1 testInput)
    
    [<Fact>]
    let ``Day 18 Puzzle 2`` () =
        Assert.Equal(3993, puzzle2 testInput)
    
    [<Theory>]
    [<InlineData("[[[[4,3],4],4],[7,[[8,4],9]]]", "[1,1]", "[[[[0,7],4],[[7,8],[6,0]]],[8,1]]")>]
    let ``Day 18 Add and Reduction`` value1 value2 expectedResult=
        Assert.Equal(expectedResult, sum value1 value2)

    [<Theory>]
    [<InlineData("[[[[[9,8],1],2],3],4]", "[[[[0,9],2],3],4]")>]
    [<InlineData("[7,[6,[5,[4,[3,2]]]]]", "[7,[6,[5,[7,0]]]]")>]
    [<InlineData("[[6,[5,[4,[3,2]]]],1]", "[[6,[5,[7,0]]],3]")>]
    [<InlineData("[[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]]", "[[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]]")>]
    [<InlineData("[[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]]", "[[3,[2,[8,0]]],[9,[5,[7,0]]]]")>]
    let ``Day 18 Explode`` value expectedResult=
        Assert.Equal(expectedResult, explode value)

    let sumTestInput = 
     [|
     "[[[0,[4,5]],[0,0]],[[[4,5],[2,6]],[9,5]]]"
     "[7,[[[3,7],[4,3]],[[6,3],[8,8]]]]"
     "[[2,[[0,8],[3,4]]],[[[6,7],1],[7,[1,6]]]]"
     "[[[[2,4],7],[6,[0,5]]],[[[6,8],[2,8]],[[2,1],[4,5]]]]"
     "[7,[5,[[3,8],[1,4]]]]"
     "[[2,[2,2]],[8,[8,1]]]"
     "[2,9]"
     "[1,[[[9,3],9],[[9,0],[0,7]]]]"
     "[[[5,[7,4]],7],1]"
     "[[[[4,2],2],6],[8,7]]"
     |]
    
    [<Fact>]
    let ``Day 18 Sum and Reduction``() =
        Assert.Equal("[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]", 
            (sumTestInput |> Array.tail |> Array.fold sum (sumTestInput|>Array.head)))

    [<Theory>]
    [<InlineData("[[[[6,6],[7,6]],[[7,7],[7,0]]],[[[7,7],[7,7]],[[7,8],[9,9]]]]", 4140)>]
    [<InlineData("[[1,2],[[3,4],5]]", 143)>]
    [<InlineData("[[[[0,7],4],[[7,8],[6,0]]],[8,1]]", 1384)>]
    [<InlineData("[[[[1,1],[2,2]],[3,3]],[4,4]]", 445)>]
    [<InlineData("[[[[3,0],[5,3]],[4,4]],[5,5]]", 791)>]
    [<InlineData("[[[[5,0],[7,4]],[5,5]],[6,6]]", 1137)>]
    [<InlineData("[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]", 3488)>]
    let ``Day 18 Magnitute`` value expectedResult=
        Assert.Equal(expectedResult, value |> parseSnailfishNumber |> magnitute)